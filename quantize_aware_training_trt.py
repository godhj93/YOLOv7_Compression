...
def train(hyp, opt, device, tb_writer=None):
    ...
    with torch.no_grad():
        ...
        jit_model = torch.jit.trace(model.to(device), images.to(device), strict=True)
    ...
    trt_mod = torch_tensorrt.compile(model, **complie_spec)
    torch.jit.save(trt_mod, weights.replace('.pt', '_qat.trt'))
    if rank in [-1, 0]:
        # Plots
        if plots:
